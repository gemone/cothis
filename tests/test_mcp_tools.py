...
def test_yaml_type_mcp_stdio_routes_to_server() -> None:
    ...
    assert server._params.command == "uvx"
    assert server._params.args == ["browser-use", "--mcp"]


def test_yaml_mcp_env_stored_on_params() -> None:
    """``env:`` is carried onto the subprocess params (secrets)."""
    yaml_text = "type: mcp.stdio\ncommand: srv\nenv:\n  API_KEY: s3cr3t\n"
    server = load_yaml_tools(yaml_text)[0]
    assert isinstance(server, _MCPServer)
    assert server._params.env == {"API_KEY": "s3cr3t"}
...
@pytest.mark.asyncio
async def test_default_connect_delivers_env_to_stdio_client(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """``_default_connect`` (the production transport, no ``connect`` seam)
    hands the declared ``env`` straight to ``stdio_client`` — so a server's
    secrets reach its subprocess (story 32). Patches the lazy transport
    imports so nothing is actually spawned."""
    import mcp
    ...
    monkeypatch.setattr(mcp, "ClientSession", FakeSession, raising=False)
    monkeypatch.setattr(mcp_stdio, "stdio_client", fake_stdio_client, raising=False)

    params = StdioServerParameters(
        command="srv", args=["--x"], env={"API_KEY": "s3cr3t"}
    )
    server = _MCPServer(name="mcp:t", params=params)  # no connect seam
    try:
...
async def test_start_failure_logs_warning_returns_empty(
    ...
    params = StdioServerParameters(
        command="badcmd", args=["--flag"], env={"SECRET_KEY": "topsecret"}
    )
    server = _MCPServer(name="broken", params=params, connect=lambda: _FailingCM())
