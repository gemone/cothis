...
class CothisApp(App):
    ...
    def on_new_session(self, worktrees: list) -> None:
        ...
        session bound to this cwd". Slice D (this PR) introduces the
        hook as a logging stub; slice E (CLI integration) overrides it
        to call ``Supervisor.spawn_worker`` + ``SessionStorage.new`` +
        ``attach_session_ws``.
